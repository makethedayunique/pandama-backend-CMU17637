...
var page = 2;
var endFlag = false;
function scroll_load(url, size, div_scroll_locator, scroll_height) {
...
function page_scroll_detector(url, page_size, total_size, div_scroll_locator) {
  ...
  if(($(window).scrollTop() + 100) >= $(document).height() - $(window).height()) {        
    if ($("#loading_div").css("display") == "none") {
      scroll_load(url, page_size, div_scroll_locator, scroll_height - 5)
    }
  }
}
